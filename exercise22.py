...
def costFunctionReg(theta, X, y, lambda_):
    ...
    # ===================== YOUR CODE HERE ======================
    J=-(np.dot(y,np.log(sigmoid(np.dot(X, theta))))+np.dot((1-y),np.log(1-sigmoid(np.dot(X,theta))))-1/2 * 
        lambda_ * np.sum(np.square(theta[1:]))) / m
    grad=(np.dot(X.T, (sigmoid(np.dot(X, theta))-y)) +lambda_ * (np.insert(theta[1:],0,0)))/m
    # =============================================================
    return J, grad
